...
final class Politis {
	...
	private function setup() {
		...
		$enqueu = new Politis_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->politis_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'politis_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'politis', POLITIS_DIR_PATH . '/languages' );
		...
		add_image_size( 'politis_blogsect_post_thumb', 295, 211, true );
		
		// Custom thumbnails size for widget post
		add_image_size( 'politis_widget_post_thumb', 75, 75, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'politis-np-thumb', 59, 59, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'politis' ),
            'social-menu'   => esc_html__( 'Footer Social Menu', 'politis' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = POLITIS_DIR_CSS_URI;
		$jsPath  = POLITIS_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					...
					'dependency' 	=> array(),
					'version' 		=> $this->politis_version,
				),
				array(
					'handler'		=> 'politis-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->politis_version,
				),
				array(
					'handler'		=> 'politis-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'politis' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate politis theme customizer
		$politis_theme_customizer = new politis_theme_customizer();
	}
	...
	public function set_elementor_flag() {
		update_option( 'politis_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'politis-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'politis_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'politis-elementor-default-style',  POLITIS_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->politis_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'politis_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'politis-elementor-notice', POLITIS_DIR_JS_URI.'politis-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'politis-elementor-notice',
				'politisElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'politis-elementor-notice-nonce' ),
				)
			);
		}
